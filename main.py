...
print('size of training data', len(dataloaders['training']))

for epoch in range(opt.n_epochs):
    m = m0*2
    print('m:', m)
    if m > len(dataloaders['training']):
    ...
    loss_collect = []
    grad_l2, grad_max = [], []
    data_iterator = tqdm(sampledata, desc='Epoch {} Training...'.format(epoch))
    for i,(class_labels, input) in enumerate(data_iterator):
        ...
        if i==len(sampledata)-1: data_iterator.set_description('Epoch (Train) {0}: Mean Loss [{1:.4f}]'.format(epoch, np.mean(loss_collect)))
